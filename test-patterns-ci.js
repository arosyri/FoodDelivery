...
async function testAllPatterns() {
    ...
    log.title('6. COMMAND (мок-версія)');
    try {
        const { CancelOrderCommand, CommandInvoker } = require('./patterns/command/CancelOrderCommand');
        let cancelled = false;
        const mockOrder = { status: 'pending', save: async () => { cancelled = true; } };
        const MockModel = { findOne: async () => mockOrder };
        const invoker = new CommandInvoker();
        const command = new CancelOrderCommand(MockModel, '123', 'user');
        const result = await invoker.executeCommand(command);
        results.command = result.success === true && cancelled === true;
        log.success(results.command ? 'Command працює (мок-БД)' : 'Command не працює');
    } catch (error) {
    ...
    log.title('8. FACADE (мок-версія)');
    try {
        const OrderFacade = require('./patterns/facade/OrderFacade');
        const facade = new OrderFacade();
        results.facade = facade && typeof facade.createEnhancedOrder === 'function';
        log.success(results.facade ? 'Facade працює (клас доступний)' : 'Facade не працює');
    } catch (error) {
        log.error(`Facade помилка: ${error.message}`);
        results.facade = false;
    }
